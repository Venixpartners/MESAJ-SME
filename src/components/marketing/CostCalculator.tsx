 ...
 *
 * Billing is per recipient, not per segment, which is why segments are
 * shown as information about the message rather than as part of the sum.
 */
...
export function CostCalculator() {
  ...
  const total = safeRecipients * PRICE_PER_SMS;
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div>
          ...
          <p className="mt-2 text-xs text-[var(--color-ink-500)]">
            ...
            {segmentInfo.encoding === "UCS2" ? ". Emoji and curly quotes shorten each part to 70 characters." : ""}
          </p>
        </div>
        ...
        <div className="flex flex-col justify-between rounded-[var(--radius-sm)] bg-[var(--color-ink-50)] p-5">
          <div>
            ...
            <p className="mt-2 text-sm leading-relaxed text-[var(--color-ink-600)]">
              {naira(PRICE_PER_SMS)} for each customer you reach. No monthly fee, no setup fee and no minimum order.
            </p>
          </div>
